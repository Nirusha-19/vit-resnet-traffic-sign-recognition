...
from common import VIT_MODEL_NAME, VIT_CHECKPOINT, VIT_RESULTS, DEVICE, NUM_CLASSES
...
def main():
    ...
    print(f"Loading pretrained {VIT_MODEL_NAME}...")
    ...
    model.to("cpu")
    os.makedirs(os.path.dirname(VIT_CHECKPOINT), exist_ok=True)
    torch.save(model.state_dict(), VIT_CHECKPOINT)
    print(f"\nSaved model to {VIT_CHECKPOINT}")

    save_results({**test_results, "best_epoch": best_epoch, "best_val_macro_f1": best_val_f1}, VIT_RESULTS)
